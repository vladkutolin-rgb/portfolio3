...
app = Flask(__name__)
app.secret_key = 'portfolio-secret-key-2025'
...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        cats = ['Программирование и разработка', 'Офисные приложения', 'Графика и дизайн', 'Другие инструменты']
...
def api_gallery_add():
    ...
    c.execute('INSERT INTO gallery (photo, description) VALUES (?,?)',
              (f'static/uploads/{fn}', request.form.get('description', '')))
...
def api_certs_add():
    ...
    c.execute('INSERT INTO certificates (photo,title,description,organization,year,course) VALUES (?,?,?,?,?,?)',
              (f'static/uploads/{fn}', request.form.get('title',''), request.form.get('description',''),
               request.form.get('organization',''), request.form.get('year',''), request.form.get('course','1')))
    conn.commit(); conn.close()
    return jsonify({'success': True})
...
def api_works_add():
    ...
    c.execute('INSERT INTO works (title,description,icon,file_path) VALUES (?,?,?,?)',
              (request.form.get('title',''), request.form.get('description',''),
               request.form.get('icon','fa-code'), fp))
    conn.commit(); conn.close()
    return jsonify({'success': True})
...
def api_cats_upd(id):
    ...
    c.execute('UPDATE categories SET name=?, sort_order=? WHERE id=?',
              (d.get('name',''), d.get('sort_order',0), id))
    conn.commit(); conn.close()
    return jsonify({'success': True})
...
def api_ts_add():
    ...
    c.execute('INSERT INTO techstack (category,name,icon,percent,subtitle,sort_order) VALUES (?,?,?,?,?,?)',
              (d.get('category',''), d.get('name',''), d.get('icon','fa-code'),
               d.get('percent',50), d.get('subtitle',''), mo))
    conn.commit(); conn.close()
    return jsonify({'success': True})
...
def api_ts_upd(id):
    ...
    c.execute('UPDATE techstack SET name=?,icon=?,percent=?,subtitle=?,category=?,sort_order=? WHERE id=?',
              (d.get('name',''), d.get('icon',''), d.get('percent',50),
               d.get('subtitle',''), d.get('category',''), d.get('sort_order',0), id))
    conn.commit(); conn.close()
    return jsonify({'success': True})
...
def api_music():
    conn = sqlite3.connect('portfolio.db'); c = conn.cursor()
    c.execute('SELECT * FROM music ORDER BY sort_order')
