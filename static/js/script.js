...
function animateOnScroll() {
    document.querySelectorAll('.animate-on-scroll').forEach(el => {
        if (el.getBoundingClientRect().top < window.innerHeight - 150) {
            ...
            if (el.id === 'about') {
                setTimeout(() => { animateTechBars(); animateCounters(); }, 300);
            }
        }
    });
}
...
// ═══════════════════════════════════════════
// АНИМАЦИИ ТЕХ-СТЕКА И СЧЁТЧИКОВ
// ═══════════════════════════════════════════
...
function updateBass() {
    if (!analyser || !isMusicPlaying) {
        ...
        window.symphonyMid = window.symphonyMid * 0.9 + 0.2 * 0.1;
        window.symphonyHigh = window.symphonyHigh * 0.9 + 0.1 * 0.1;
        window.symphonyBeat = 0;
        return;
    }
    ...
    window.symphonyBass = window.symphonyBass * 0.65 + rawBass * 0.35;
    window.symphonyMid = window.symphonyMid * 0.65 + rawMid * 0.35;
    window.symphonyHigh = window.symphonyHigh * 0.65 + rawHigh * 0.35;
    window.symphonyBeat = rawBass > 0.45 ? 1 : 0;
}
...
function initGrotto() {
    ...
    function drawRiver() {
        ...
        const riverY = H * 0.68;
        const riverSpeed = 1.5 + bass * 4 + beat * 2;
        ...
        for (let x = 0; x < W; x += 2) {
            ...
            const y1 = riverY +
                Math.sin((x + offset) / 70) * (4 + bass * 15 + beat * 10) +
    ...
    function drawCeiling() {
        ...
        lw.addColorStop(0, 'rgba(15,15,25,0.8)');
        lw.addColorStop(1, 'rgba(15,15,25,0)');
        ctx.fillStyle = lw;
        ctx.fillRect(0, 0, W * 0.08, H);

        const rw = ctx.createLinearGradient(W, 0, W * 0.92, 0);
        rw.addColorStop(0, 'rgba(15,15,25,0.8)');
        rw.addColorStop(1, 'rgba(15,15,25,0)');
        ctx.fillStyle = rw;
...
function initAbyss() {
    ...
    let shards = [], abyssCrystals = [], stardust = [];
    ...
    function drawShards(bass) {
        shards.forEach(s => {
            ...
            ctx.lineTo(-s.size / 2, s.size * 0.3);
            ctx.closePath();
            ctx.fill();
            ctx.restore();
            ...
            ctx.beginPath();
            ctx.arc(s.x - s.size * 0.15, s.y - s.size * 0.2, s.size * 0.15, 0, Math.PI * 2);
            ctx.fill();
        });
    }
    ...
    function drawAbyssCrystals(bass) {
        abyssCrystals.forEach(c => {
            ...
            for (let l = 3; l >= 0; l--) {
                ...
                gl.addColorStop(0, `hsla(${c.hue}, 60%, 60%, ${la * 2})`);
            ...
            c.rings.forEach(ring => {
                ...
                ctx.strokeStyle = `hsla(${c.hue}, 60%, 65%, ${alpha * 0.5})`;
                ctx.lineWidth = 1.2;
                ctx.shadowColor = `hsla(${c.hue}, 70%, 55%, ${alpha * 0.4})`;
                ctx.shadowBlur = 6;
                ctx.beginPath();
            ...
            core.addColorStop(0.5, `hsla(${c.hue}, 50%, 65%, 0.7)`);
            core.addColorStop(1, 'rgba(0,0,0,0)');
            ctx.fillStyle = core;
            ctx.beginPath();
            ctx.arc(c.x, c.y, c.size * 0.25, 0, Math.PI * 2);
            ctx.fill();
            ...
            c.trail.forEach((t, i) => {
                t.life -= 0.05;
                if (t.life > 0) {
                    ctx.fillStyle = `hsla(${c.hue}, 50%, 55%, ${t.life * 0.08})`;
                    ctx.beginPath();
                    ctx.arc(t.x, t.y, c.size * 0.1 * (i / 6) * t.life, 0, Math.PI * 2);
                    ctx.fill();
                }
            });
            ...
            if (Math.random() < 0.25 + c.bassMemory * 0.4) {
                c.sparkles.push({
                    x: c.x + (Math.random() - 0.5) * c.size * 0.4,
                    y: c.y + (Math.random() - 0.5) * c.size * 0.2,
                    vx: (Math.random() - 0.5) * 0.4,
                    vy: -0.3 - Math.random(),
                    life: 1,
            ...
            c.sparkles.forEach(p => {
                ...
                if (p.life > 0) {
                    ctx.fillStyle = `rgba(200, 220, 255, ${p.life * 0.7})`;
                    ctx.beginPath();
                    ctx.arc(p.x, p.y, p.size * p.life, 0, Math.PI * 2);
                    ctx.fill();
                }
...
function initDogBoat() {
    ...
    function setPos(x, y) {
        ...
        x = Math.max(30, Math.min(x, r.width - 80));
        y = Math.max(50, Math.min(y, r.height - 80));
        dog.style.left = x + 'px';
        dog.style.top = y + 'px';
    ...
    function physics() {
        const r = gallery.getBoundingClientRect();
        let x = parseFloat(dog.style.left) || r.width / 2;
        let y = parseFloat(dog.style.top) || r.height * 0.65;
        x += vx; y += vy;
        vx *= 0.93; vy *= 0.93;
    ...
    let autoAngle = 0;
    setInterval(() => {
        if (!dragging && (!animId || Math.abs(vx) < 0.08)) {
            autoAngle += 0.01;
            const r = gallery.getBoundingClientRect();
            setPos(r.width / 2 + Math.sin(autoAngle) * 60, r.height * 0.65 + Math.cos(autoAngle * 0.6) * 10);
        }
    }, 50);

    setTimeout(() => {
        const r = gallery.getBoundingClientRect();
        setPos(r.width / 2, r.height * 0.65);
    }, 500);
}
