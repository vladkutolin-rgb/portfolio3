...
// ═══════════════════════════════════════════
// ВОДОПАДЫ + РЕКА — ULTIMATE EDITION
// ═══════════════════════════════════════════
function initWaterfallGame() {
    ...
    function drawRocks() {
        ...
        lg.addColorStop(0, 'rgba(45, 50, 55, 0.4)');
        lg.addColorStop(0.5, 'rgba(55, 60, 65, 0.2)');
        lg.addColorStop(1, 'rgba(60, 65, 70, 0)');
        ctx.fillStyle = lg;
        ctx.beginPath();
        ...
        rg.addColorStop(0, 'rgba(45, 50, 55, 0.4)');
        rg.addColorStop(0.5, 'rgba(55, 60, 65, 0.2)');
        rg.addColorStop(1, 'rgba(60, 65, 70, 0)');
        ctx.fillStyle = rg;
        ctx.beginPath();
    ...
    function drawWaterfall(particles, x, width) {
        ...
        mist.addColorStop(0, 'rgba(200, 230, 255, 0.12)');
        mist.addColorStop(0.4, 'rgba(160, 210, 245, 0.15)');
        ...
        particles.forEach(p => {
            ...
            ctx.beginPath();
            ctx.arc(p.x - p.size * 0.2, p.y - p.size * 0.35, p.size * 0.35, 0, Math.PI * 2);
            ctx.fill();
        });
    }
    ...
    function drawSplashes() {
        splashes.forEach(d => {
            ...
            d.life -= 0.014;
            if (d.life > 0) {
                ctx.fillStyle = `rgba(210, 240, 255, ${d.life * 0.7})`;
                ctx.beginPath();
    ...
    window.triggerTsunami = function() {
        window.tsunamiHeight = 55;
        const wave = document.getElementById('tsunamiWave');
        if (wave) {
            wave.style.height = '110px';
            wave.style.background = 'linear-gradient(0deg, rgba(0,210,255,0.8), rgba(0,170,230,0.5), rgba(255,255,255,0.3), transparent)';
            setTimeout(() => wave.style.height = '0', 1400);
        }
    };
}
